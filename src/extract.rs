//! Putting the content file where the target application can open it.
//
// Author: David M. Anderson
// Built with AI assistance (Claude, Anthropic)
//
//! Concept 5, steps 3 and 4: the content file is written into the session's own
//! `content/` directory (concept 6.4) and then carries whatever the platform
//! records about where the container came from.
//!
//! **A content file the mark could not be carried onto does not survive this
...
//! anything is told the content file exists.
...
#[derive(Debug)]
pub enum Error {
    /// The content file could not be read out of the container: encrypted,
    /// stored with a compression method this build carries no decoder for, or a
    /// container declaring a version it does not implement.
    Unreadable(slpc::Error),
    /// The content file could not be written.
    Write(slpc::Error),
    /// Where the container came from could not be carried onto the content
    /// file, so opening the content file would not raise the warning opening
    ...
    Unmarked {
        ...
        cause: slpc::Error,
        /// Whether the content file was removed again. False means it is on
        /// disk and ungated, which the caller has to say out loud.
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Unreadable(e) => write!(f, "the content file cannot be read: {e}"),
            Self::Write(e) => write!(f, "the content file could not be written: {e}"),
            Self::Unmarked {
                cause,
                content_removed,
            } => write!(
                f,
                "where the container came from could not be carried onto its content file: \
                 {cause}\n\
                 The content file {}, because opening it would not raise the warning the \
...
/// Write the content file into `session`, then carry the container's mark onto
/// it.
///
...
///
/// The content file gets the permissions a newly created file would ordinarily
...
/// answer, and it never consults `content_mode`.
///
/// # Errors
///
/// See [`Error`]. Every variant means the content file must not be launched,
/// and the [`Error::Unmarked`] one means it must not be left behind either.
pub fn extract<R: Read + Seek>(
    container: &mut Container<R>,
    session: &mut Session,
) -> Result<Mark, Error> {
    // Asked before anything is written, so an encrypted content file or one
    // stored by a method this build cannot decode is a sentence rather than a
    ...
    let out = session.content_path();
    // No-clobber. The session directory was made for this content file a
    // moment ago, so anything already under the name is something else's, and
    ...
    {
        let mut content = container.content().map_err(Error::Unreadable)?;
        std::io::copy(&mut content, dest.writer()).map_err(|e| Error::Write(e.into()))?;
    }
    ...
    if let Ok(crc) = container.content_crc() {
    ...
    match slpc::provenance::carry(&session.record().container, &out) {
        ...
        Err(cause) => Err(Error::Unmarked {
            content_removed: remove(&out),
            cause,
        }),
    }
}

/// Take the content file back off disk. Reported rather than propagated: the
/// caller is already failing, and whether the ungated file is still there
/// changes what the sentence has to say rather than whether there is one.
...
mod tests {
    ...
    /// A container on disk holding `content_bytes` under `name`.
    fn container(at: &Path, name: &str, content_bytes: &[u8]) -> PathBuf {
        ...
        slpc::pack_reader(name, content_bytes, doc, out).unwrap();
        path
    }
    ...
    #[test]
    fn the_content_file_lands_under_its_own_name_with_its_own_bytes() {
        ...
        assert_eq!(s.content_path().file_name().unwrap(), "report.pdf");
        assert_eq!(
            fs::read(s.content_path()).unwrap(),
            b"%PDF-1.7 not really\n"
        );
    }

    #[test]
    fn a_zero_length_content_file_is_written_rather_than_refused() {
        ...
        extract(&mut open(&c), &mut s).unwrap();
        assert_eq!(fs::read(s.content_path()).unwrap(), b"");
    }

    #[test]
    fn the_content_file_is_the_only_thing_written_into_the_content_directory() {
        ...
        // content file is written when extracting.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("sessions");
        ...
        let mut found: Vec<_> = fs::read_dir(s.content_dir())
            .unwrap()
    ...
    fn extracting_twice_into_one_session_refuses_rather_than_replaces() {
        ...
        // content file somebody may be editing.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("sessions");
        ...
        let mut s = session::create(&root, &c, "report.pdf").unwrap();
        extract(&mut open(&c), &mut s).unwrap();
        fs::write(s.content_path(), b"edited by somebody").unwrap();

        assert!(matches!(
            extract(&mut open(&c), &mut s),
            Err(Error::Write(_))
        ));
        assert_eq!(fs::read(s.content_path()).unwrap(), b"edited by somebody");
    }

    #[test]
    fn a_container_that_arrived_from_elsewhere_marks_the_content_file_it_yields() {
        // The reason this step exists. Unpacking without it is laundering: the
        // content file reaches its handler as something this machine made, and
        // the warning the platform would have shown never appears.
        //
        ...
        assert!(slpc::provenance::arrived_from_elsewhere(&s.content_path()));
    }

    #[test]
    fn a_container_that_says_nothing_yields_a_content_file_that_says_nothing() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("sessions");
        let c = container(tmp.path(), "report.pdf", b"%PDF");
    ...
    #[test]
    fn an_unreadable_content_file_is_refused_before_anything_is_written() {
        // An encrypted content file, which SPEC 2.5 forbids rejecting the
        // container over and which this build cannot decode. The refusal is a
        // sentence rather than a half-written file in the session directory.
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("sessions");
        ...
        assert!(!s.content_path().exists());
        assert_eq!(fs::read_dir(s.content_dir()).unwrap().count(), 0);
    }

    /// A container whose content file sets general purpose bit 0, which is what
    /// `check_content_readable` refuses. Built by hand, because the writer will
    /// not produce one.
    fn encrypted_container() -> Vec<u8> {
        let doc = "slipcase_version = \"1.1\"\n\n[content]\nfile = \"secret.pdf\"\n";
    ...
    /// Turn on general purpose bit 0 on the content file's local file header
    /// and its central directory entry, and on neither of the flyleaf member's.
    ///
    /// The name is read at the offset the header says it is at, rather than
    /// searched for. Searching finds `secret.pdf` after the flyleaf header too,
    /// which sets the flag on the flyleaf member and makes the container
    /// undetermined under SPEC 2.2 instead of holding an unreadable content
    /// file — a different refusal, arriving before the one this test is about.
